          ...
          <template v-if="isTableLoading">
            <div v-for="i in 5" :key="'mob-skel-'+i" class="p-4 sm:p-5 animate-pulse space-y-4">
              ...
              <div class="flex justify-between items-end">
                <div class="h-6 bg-slate-100 dark:bg-slate-800/50 rounded-md w-24"></div>
                <div class="h-5 bg-slate-200 dark:bg-slate-700/50 rounded w-16"></div>
              </div>
            </div>
          </template>
          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
              ...
              <tr v-if="isTableLoading" v-for="i in 6" :key="'desk-skel-'+i" class="animate-pulse bg-white/30 dark:bg-slate-900/30">
                ...
                <td class="px-6 py-4"><div class="h-6 bg-slate-200 dark:bg-slate-700/50 rounded-md w-24"></div></td>
                <td class="px-6 py-4"><div class="h-4 bg-slate-200 dark:bg-slate-700/50 rounded w-16"></div></td>
              ...
              >
                <!-- First cell gets 'relative' for the highlight line (Compliant with HTML spec) -->
...
import { Search, Plus, Trash2, ChevronLeft, ChevronRight, PackageOpen, Tag } from "lucide-vue-next"
...
const EmptyStateContent = (props: { isReadOnly: boolean }, { emit }: any) => {
  return h('div', { class: 'flex flex-col items-center justify-center space-y-4' }, [
    h('div', { class: 'flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 shadow-inner' }, [
      h(PackageOpen, { class: 'w-8 h-8 opacity-80' })
    ]),
    h('div', { class: 'text-center' }, [
      h('h3', { class: 'text-lg font-bold text-slate-900 dark:text-white' }, 'No items found'),
      h('p', { class: 'text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto' }, 
        props.isReadOnly 
          ? 'There are currently no items matching your search.' 
...
const onItemCreated = (item: Item) => {
  ...
  totalItems.value++
  navigateToItem(item.id) // Auto-navigate consistency across your models
}
