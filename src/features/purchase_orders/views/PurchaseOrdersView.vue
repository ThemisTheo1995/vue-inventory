...
const formatSupplierName = (supplier?: { name?: string; first_name?: string; last_name?: string | null } | null) => {
  if (!supplier) return "Unknown Supplier"
  if (supplier.name) return supplier.name
  const first = supplier.first_name || ""
  const last = supplier.last_name || ""
  const fullName = `${first} ${last}`.trim()
  return fullName || "Unnamed Supplier"
}
...
const updateStatus = async (po: PurchaseOrder, nextStatus: PurchaseOrderStatus) => {
  const isTransitionConfirmed = await confirm({
    ...
    message: `Are you sure you want to transition this purchase order status to ${nextStatus}? This action dictates downstream order processing.`,
  ...
  try {
    await purchaseOrderService.update(workspaceId, po.id, { status: nextStatus })
